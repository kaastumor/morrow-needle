...
Issue #29's micro→macro adversary has reached its gate decision:

> **SIMPLIFY. Do not answer a failed broad novelty claim with more analytic
> primitives. Keep the evidence engine; make the next horizon prove comparative
> user value.**

The operational loop is real and the P0/P1 truth boundaries survived repeated
executable attacks after several high-severity enforcement repairs. The audit
also rejected monitoring/versioning/diffing/grounded explanation, temporal legal
graphs, semantic extraction, corrigenda analysis and dependency propagation as
standalone Needle novelty claims.

The highest-value unknown is now:

> **Does Needle's stricter evidence/legal-state composition help a person reach a
> materially more correct understanding than strong existing tools plus direct
> official sources and an ordinary capable LLM?**

Unless new official/executable evidence reopens a foundation risk:

1. **P2 / Issue #49 — Comparative value proof: Evidence / Why this is shown. NEXT HORIZON.** Build the smallest human-readable evidence resolver over existing provenance, then compare difficult real cases against the strong simpler baseline. No new legal ontology, feed-card schema, database or frontend framework unless a real case proves the current baseline insufficient.
2. **Gate after #49:** choose continue / simplify / redirect / stop from comparative evidence. No affected-entity model, Half-Life v0.2, ranking, subscriptions or new analytic primitive may leapfrog this value proof.
3. **Parked:** Half-Life v0.2 / Issue #19; broader ranking; generic “AI explanation”; domain subscriptions; Dark Matter expansion; wider jurisdictions. Reopen only from evidence generated by the active horizon.

Delivery policy entering #49:

- `CHANGE_FEED` is candidate public-news input.
- `AUDIT_FEED` and `ABSTENTION_FEED` remain durable, inspectable truth but are
  contextual evidence/monitor channels by default, not general-interest news.
- Public wording should use **Evidence / Why this is shown**, not internal
  “Source Mode” terminology.
- Dependency Ripple/X-Ray, Half-Life v0.1 and Source Anomaly v0.1 remain valid
  frozen projections; they are not an active feature ladder.
...
Canonical pre-analysis audit: `docs/audits/pre-analysis-adversarial-audit-v0.1.md`  
Gate health decision: `docs/audits/project-health-2026-09-23.md`  
...
Current horizon after Issue #29:

1. **Comparative value proof / Issue #49.** Resolve canonical provenance into a
   human-readable Evidence view and test difficult real cases against the strong
   simpler baseline.
2. **Project Health gate.** Do not assume success. If Needle does not repeatedly
   prevent material misunderstanding or expose useful legal-state distinctions,
   simplify again, redirect to a narrower research/audit tool, or stop public
   product expansion.
3. Only after that gate may a new product capability compete for architecture.
...
- **Pre-analysis adversarial audit / Issue #29: COMPLETE, direction SIMPLIFY.**
  ...
  rejected. Existing P2 analytics were reclassified by user value rather than
  architectural interest.
- **Product checkpoint v0.1 / Issue #27: COMPLETE.** Three real operational
  classes established that human-readable evidence presentation is a recurring
  comprehension gap. #29 narrows the public concept to **Evidence / Why this is
  shown** and rejects raw audit/abstention output as default general-interest
  feed content.
- **Operational Needle v0.1 / Issue #21: CLOSED.** Live official events flow
  through persistent monitoring, targeted re-observation, immutable provenance,
  authentic-cause analysis, publication relevance and evidence-linked feed
  projection.
- **Legislative X-Ray v0.1 / Issue #20: CLOSED/FROZEN.** Retained as an optional
  derived view for the real question “did an unchanged provision's operation
  change through a referenced rule?” No active v0.2 expansion.
- **Half-Life v0.1 / Issue #10: CLOSED/FROZEN.** Retained as a valid temporary-
  regime demonstration. v0.2 research is parked pending product demand.
- **Source Anomaly v0.1 / Issue #17: CLOSED/FROZEN.** Retained primarily as
  trust/audit diagnostics rather than a headline public-intelligence pillar.

Strategic constraints:

- Do not use “novel” as a proxy for useful. Most component ideas have precedent.
- Do not add another analytic primitive to rescue the thesis.
- Do not create infrastructure for hypothetical scale.
- Do not promote source/audit activity into legal news.
- Keep `docs/project-charter.md`, `docs/assumptions.md`,
  `docs/audits/project-health-2026-09-23.md` and this backlog aligned with the
  evidence produced by #49.
