...
already-in-force but editorally unapplied change to the selected provision that
...
- the frozen UK search yielded a deterministic candidate pool, but none passed
  all positive-case eligibility conditions;
...
The missing element was the fresh UK positive case, not the construct-validity
control.
