...
def instruction_identity(item: dict) -> tuple:
    """Representation duplicates do not multiply one authentic legal instruction."""
    return (item.get("operation"),item.get("target_locator"),tuple(item.get("inserted_keys",[])),item.get("placement_anchor"),item.get("instruction_text"))
...
def main() -> int:
    ...
    unique={instruction_identity(item):item for item in matches}
    if len(unique) != 1:
        raise AssertionError(f"expected one semantic US-2.1405/1406 instruction, got {len(unique)} identities from {len(matches)} representations")
    instruction=next(iter(unique.values()))
    representation_occurrences=len(matches)
    ...
    source_change={
        "change_id":f"source-change:{digest({'event_key':event['event_key'],'content_hash':content_hash,'basis':'MISSING_BASELINE'})}",
        "event_key":event["event_key"],"feed_action":event["action"],"classification":"UNRESOLVED","target_cellar_id":event["cellar_id"],"root_cellar_id":event["root_cellar_id"],"refresh_scope":"WORK","previous":None,
    ...
    downstream={
        "disposition":"LEGAL_CHANGE_VERIFIED","verification_route":"AUTHENTIC_LEGAL_CAUSE","canonical_refs":[{"kind":"MUTATION","entity_id":mutation["candidate_id"]}],
        "evidence_refs":[f"authentic-act:CELEX:{CAUSE}",f"authentic-instruction:{mutation['candidate_id']}"],
        "explanation":{"what_changed":"Annex V, Part 1, Section B adds United States zones US-2.1405 and US-2.1406 after zone US-2.1404.","compared_with":"The placement anchor US-2.1404 named by the authentic amending instruction; no before/after consolidated source pair is asserted.","when_it_matters":"The legal change is established by Regulation (EU) 2026/2104; source-state comparison remains unresolved until an evidenced comparator exists.","affected":["United States"],"evidence_character":"DIRECT"},"unknowns":[],
    }
    result=build_operational_result(event,source_change,downstream=downstream)
    card=build_feed_card(result)
    validate(result,"operational-result-v0.1.schema.json"); validate(card,"feed-card-v0.1.schema.json")

    OUT.mkdir(parents=True,exist_ok=True)
    bundle={"character":"LIVE_OFFICIAL_EVENT_TO_PUBLIC_CARD","trigger_event":event,"authentic_source":cause,"authentic_instruction":instruction,"instruction_representation_occurrences":representation_occurrences,"canonical_mutation":mutation,"source_change":source_change,"operational_result":result,"feed_card":card,"guardrail":"Duplicate Formex representations are one evidentiary instruction; CHANGE_FEED verification comes from the authentic legal cause, not the unresolved source-diff branch."}
    (OUT/"change-feed-card.json").write_text(json.dumps(bundle,indent=2,ensure_ascii=False)+"\n",encoding="utf-8")
    print(json.dumps({"event_key":event["event_key"],"mutation_id":mutation["candidate_id"],"instruction_representation_occurrences":representation_occurrences,"stream":card["stream"],"headline":card["headline"],"verification_route":card["verification_route"],"unknowns":card["unknowns"]},indent=2))
    return 0
