    ...
    for action in ACTIONS:
        for wemi in WEMI_CLASSES:
            for page_number in range(1,MAX_PAGES+1):
                ...
                payload,response=fetch_feed(params)
                ...
                raise AssertionError(
                    f"feed pagination exceeded {MAX_PAGES} pages: "
                    f"{action}/{wemi}"
                )
...
def select_representative(
    ...
    def key(event):
        relevance_rank=(
            0 if classify_event_relevance(event)
            == "LEGAL_RESOURCE_CANDIDATE" else 1
        )
        when=-instant(event["ingestion_time"]).timestamp()
        level=min(
            (wemi_rank.get(value,9) for value in event.get("wemi_levels",[])),
            default=9,
        )
...
def main() -> int:
    ...
    if errors:
        raise AssertionError(
            "invalid operational state: " + "; ".join(errors)
        )

    cursor=state["poll_cursor"]
    last=instant(cursor["last_completed_end"])
    overlap=timedelta(seconds=cursor["overlap_seconds"])
    start=last-overlap
    end=datetime.now(timezone.utc)
    if end <= last:
        raise AssertionError("monitor end must advance beyond completed cursor")

    events,feed_observations=fetch_window(start=start,end=end)
    processed=set(state.get("processed_event_keys",[]))
    new_events=[
        event for event in events
        if event["event_key"] not in processed
    ]

    grouped=defaultdict(list)
    for event in new_events:
        grouped[event["root_cellar_id"]].append(event)
    ...
    for root in sorted(grouped):
        ...
        related=[
            event["event_key"]
            for event in group
            if event["event_key"] != representative["event_key"]
        ]
        relevance=classify_event_relevance(representative)
        source_unknowns=[]

        if len(group) > 1:
            source_unknowns.append(
                "Multiple feed notifications for this root were collapsed into "
                "one current-state refresh; intermediate source states were not "
                "reconstructed in this operational cycle."
            )

        lookup=lookup_baseline(next_state,representative)
        if lookup["state"] != "ELIGIBLE":
            source_unknowns.append(lookup["reason"])
            previous=None
        else:
        ...
        if relevance == "SOURCE_INFRASTRUCTURE":
            ...
            reobservation={
                ...
                "snapshot":None,
                "unknowns":[
                    "DELETE notification is an availability hint; no current "
                    "artifact is assumed."
                ],
            }
            current=None
        else:
            reobservation=reobserve_event(
                representative,
                observed_at=end.isoformat(),
            )
            current=reobservation.get("snapshot")
            source_unknowns.extend(reobservation.get("unknowns",[]))
        ...
        group_records.append({
            ...
            "baseline_lookup_state":lookup["state"],
            "reobservation_state":reobservation["state"],
            "source_change_classification":change["classification"],
            "disposition":result["disposition"],
            "stream":card["stream"],
        })

        # Current observation becomes prospective baseline only after this
        # complete polling window. It can never solve an event inside this same
        ...
        if (
            ...
            and (
                current.get("content_observation_id")
                or current.get("metadata_observation_id")
            )
        ):
            seed_character=(
                "POST_EVENT_REFRESH"
                if lookup["state"] == "ELIGIBLE"
                else "COLD_START_SEED"
            )
            next_state=advance_baseline(
                next_state,
                representative,
                reobservation,
                updated_at=end.isoformat(),
                seed_character=seed_character,
                eligible_for_event_ingestion_after=end.isoformat(),
            )
        else:
            next_state=mark_event_processed(
                next_state,
    ...
    next_state=complete_poll_window(
        next_state,
    ...
    if errors:
        raise AssertionError(
            "invalid next operational state: " + "; ".join(errors)
        )

    card_schema=json.loads(
        Path("schemas/feed-card-v0.1.schema.json").read_text(encoding="utf-8")
    )
    for card in cards:
    ...
    report={
        ...
        "root_group_count":len(grouped),
        "groups":group_records,
        "stream_counts":{
            stream:sum(card["stream"] == stream for card in cards)
            for stream in ("CHANGE_FEED","AUDIT_FEED","ABSTENTION_FEED")
        },
        "disposition_counts":{
            disposition:sum(
        ...
        "baseline_count_before":len(state["baselines"]),
        "baseline_count_after":len(next_state["baselines"]),
        "source_observation_count_before":len(
            state["source_observations"]
        ),
        "source_observation_count_after":len(
            next_state["source_observations"]
        ),
    }
