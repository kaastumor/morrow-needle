...
def validate(instance: dict,schema_name: str) -> None:
    ...
    resolver=RefResolver(base_uri=schema.get("$id",schema_path.resolve().as_uri()),referrer=schema,store=store)
...
def main() -> int:
    ...
    result=build_operational_result(event,source_change,downstream=downstream); card=build_feed_card(result)
