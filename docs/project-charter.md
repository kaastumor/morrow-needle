...
A useful Needle investigation may answer:

- what changed, and compared with what;
- whether a source update is actually a legal mutation;
- when different legal effects begin or end;
- whether an unchanged rule is indirectly affected by something it references;
- whether a corrigendum or source anomaly matters to the scoped rule;
- how a rule became what it is today;
- what official evidence supports each claim;
- what remains unresolved;
- whether a tempting interpretation should be rejected.

A negative or non-impact result is a valid endpoint. The project no longer
requires every finding to become a public feature.
...
Needle must still compare its conclusions against the strongest practical
baseline:

1. EUR-Lex / OEIL and other official EU sources;
2. mature legal version/change tools where relevant;
...
A run is successful when it produces at least one well-supported outcome:

- verified legal-history finding;
- source/metadata contradiction;
- evidence-ownership defect;
...
- feature accumulation to create a product moat;
...
- general-purpose legal knowledge graph work;
