...
KEYED_ROW_INSERT_RE = re.compile(
    ...
    r"(?P<anchor>[A-Z]{2}-[A-Z0-9.]+)",
    re.IGNORECASE,
)


def parse_authentic_keyed_row_insertions(
    text: str,
    *,
    source_id: str,
    parent_locator: str,
    ...
    The instruction must name inserted row keys and the row after which they
    are inserted. The caller supplies the surrounding authentic subdivision.
    Returned objects are mutation instructions, not semantic Change Atoms and
    not claims about a consolidated representation.
    """
    normalized=" ".join(text.split())
    parsed=[]
    for match in KEYED_ROW_INSERT_RE.finditer(normalized):
        keys=[match.group("key1"),match.group("key2")]
        part=" ".join(match.group("part").split())
        section=" ".join(match.group("section").split())
        entry=" ".join(match.group("entry").split())
        anchor=match.group("anchor")
        target=(
            f"{parent_locator} > {part} > {section} > {entry} > "
            f"rows {keys[0]}, {keys[1]}"
        )
        parsed.append({
            "operation":"INSERT",
            "target_locator":target,
...
def candidate_from_authentic_instruction(
    ...
    *,
    ...
    No before/after text state is fabricated. Only explicit textual
    INSERT/DELETE/REPLACE instructions are eligible.
    """
    evidence=instruction.get("evidence")
    if not isinstance(evidence,dict):
        raise ValueError("authentic instruction lacks evidence")
    if evidence.get("channel") != "AUTHENTIC_ACT":
        raise ValueError("candidate requires AUTHENTIC_ACT evidence")
    if evidence.get("authority_character") != "CANONICAL_LEGAL_CAUSE":
        raise ValueError("candidate requires canonical legal cause")
    operation=instruction.get("operation")
    if operation not in {"INSERT","DELETE","REPLACE"}:
        raise ValueError(f"unsupported authentic-only operation: {operation}")
    target=instruction.get("target_locator")
    if not target or target != evidence.get("target_locator"):
        raise ValueError(
            "instruction target must exactly match authentic evidence target"
        )
    if evidence.get("operation") != operation:
        raise ValueError(
            "instruction operation must exactly match authentic evidence"
        )
    ...
    if candidate_id is None:
        material="|".join([
            ...
            operation,
            target,
            evidence.get("locator") or "",
        ])
        digest=hashlib.sha256(material.encode("utf-8")).hexdigest()[:20]
        candidate_id=f"authentic-mutation:{digest}"
    ...
    return {
        ...
        "target":{
            ...
            "parent_citation_path":(
                " > ".join(target.split(" > ")[:-1]) or None
            ),
            "language":language,
        },
        "alignment_basis":"SOURCE_NATIVE_IDENTIFIER",
        "before":None,
        "after":None,
        "feature_deltas":{
            "numbers_added":[],
        ...
        "reconciliation_state":"AUTHENTIC_CAUSE_ONLY",
        "verification_state":"VERIFIED",
        "supporting_evidence":[evidence],
        "conflicting_evidence":[],
        "notes":(
            "Verified directly from an explicit authentic amendment "
            "instruction. No consolidated after-state is asserted."
        ),
        "candidate_origin":"AUTHENTIC_INSTRUCTION",
    }
