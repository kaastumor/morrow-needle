...
# Reuse the live official-source probe rather than creating a second retrieval path.
...
def main() -> int:
    ...
    text=cause.pop("text")
    instructions=parse_authentic_keyed_row_insertions(
        text,
        source_id=f"CELEX:{CAUSE}",
        parent_locator=PARENT,
        locator=cause["final_url"],
    )
    if len(instructions) != 1:
        raise AssertionError(f"expected one explicit keyed-row insertion, got {len(instructions)}")
    instruction=instructions[0]
    if instruction["inserted_keys"] != ["US-2.1405","US-2.1406"] or instruction["placement_anchor"] != "US-2.1404":
        raise AssertionError("live authentic instruction no longer matches the evidenced adversary")

    mutation=candidate_from_authentic_instruction(
        instruction,kind="TABLE_ROWS",language="eng",
        candidate_id="mutation:reg2026-2104-annex-v-us-zones-1405-1406",
    )
    ...
    source_change={
        "change_id":f"source-change:{digest(change_material)}",
        ...
        "previous":None,
        "current":{
            "available":True,"content_hash":content_hash,"metadata_hash":None,
            "content_observation_id":observation_id,"metadata_observation_id":None,
        },
        "classification_basis":["FEED_ACTION","MISSING_BASELINE"],
        "notes":"Live official UPDATE re-observed successfully; no pre-event comparator is asserted.",
    }
    ...
    downstream={
        ...
        "evidence_refs":[
            f"authentic-act:CELEX:{CAUSE}",
            f"authentic-instruction:{mutation['candidate_id']}",
        ],
        "explanation":{
    ...
    bundle={
        "character":"LIVE_OFFICIAL_EVENT_TO_PUBLIC_CARD",
        "trigger_event":event,
        "authentic_source":cause,
        "authentic_instruction":instruction,
        "canonical_mutation":mutation,
        "source_change":source_change,
        "operational_result":result,
        "feed_card":card,
        "guardrail":"The CHANGE_FEED disposition is verified by the authentic legal cause, not by the unresolved source-diff branch.",
    }
